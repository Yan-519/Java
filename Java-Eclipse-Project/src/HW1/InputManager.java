...
public class InputManager {
	...
	public static final String ALL = "ALL",  POSITIVE = "POSITIVE", NOT_NEGATIVE = "NOT_NEGATIVE";
	...
	public static int inputInt(String text, boolean withBack) {
		return inputInt(text, ALL, withBack);
	}
	// returns an integer from the console
	public static int inputInt(String text) {
		return inputInt(text, ALL, true);
	}
	// returns an integer from the console with a spasific sign (inputSign)
	public static int inputInt(String text, String inputSign) {
	...
	public static int inputInt(String text, String inputSign, boolean withBack){
		int out;
	...
	public static double inputDouble(String text, String inputSign, double max) {
		double out;
		while (max < (out = inputDouble(text, inputSign)) && max != BACK);
		return out;
	}
	// returns an double from the console with a spasific sign (inputSign)
	public static double inputDouble(String text, String inputSign) {
		double out;
	...
	public static boolean inputBool(String text) {
		text += " (Yes/No): ";
	    String in = inputString(text, false, false);
	    while (!in.equalsIgnoreCase("Yes") && !in.equalsIgnoreCase("No")) {
			in = inputString(text, false, false);
		}
	...
	// retruns a String with a stop condition that can be turned off
	public static String inputString(String text, boolean isName, boolean withBack) {
		if(isName)
		...
		while (true) {
			...
			else if(withBack &&  str.equalsIgnoreCase(Integer.toString(BACK))) 
				return str;
	...
	public static Customer creatCustomer(int code) {
	    ...
	    int zip = inputInt("Enter new ZIP code (positive)", POSITIVE);
	...
	public static Restaurant createRestaurant(int code) {
	    ...
	    double rating = inputDouble("Enter rating (0 to 5)", NOT_NEGATIVE, 5);
	    if (rating == BACK) return null;

	    double fee = inputDouble("Enter base delivery fee (not negative)", NOT_NEGATIVE);
	...
	public static FastFoodRestaurant createFastFoodRestaurant(int code) {
	    ...
	    int prepTime = inputInt("Enter average preparing time (not negative)", NOT_NEGATIVE);
	    if (prepTime == BACK) return null;

	    double expressCost = inputDouble("Enter additional cost for express delivery (not negative)", NOT_NEGATIVE);
	...
	public static PremiumRestaurant createPremiumRestaurant(int code) {
	    ...
	    double minOrder = inputDouble("Enter minimum order cost (not negative)", NOT_NEGATIVE);
	    if (minOrder == BACK) return null;

	    double commission = inputDouble("Enter additional commission percentage per order (not negative)", NOT_NEGATIVE);
